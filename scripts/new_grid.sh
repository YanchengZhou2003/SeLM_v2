for N in 1024 2048 4096 8192 16384; do
    for tp in 3 6 10; do
        for t in 2 10 6; do
            for ratio_dyn in 0.99 0.1 0.3 0.6 0.9 ; do
                if [ $N -eq 16384 ]; then
                    block_size=16
                elif [ $N -eq 8192 ]; then
                    block_size=32
                ...
                python -m src.gpt  \
                    ...
                    --h 14 --tp $tp --c 1.0 --cur_tp 2 --cur_portion 1.00  \
                    ...
                    --vis_path ./vis/vis_ICML/N${N}_tp${tp}_temp${t}_ratio${ratio_dyn}/ \
                    --use_filter 0 \
                    --train_save_path ./ckpt/cte/N${N}_tp${tp}_temp${t}_ratio${ratio_dyn}.pt \
                    > ./logs/log_ICML/N${N}_tp${tp}_temp${t}_ratio${ratio_dyn}.log 2>&1
                wait
